...
void ReadFile(const std::string& filePath)
{
	...
	while (std::getline(file, line))
	{
		...
		if (line.find("#Light") != std::string::npos)
		{
			if (light != nullptr)
			{
				light->transform.SetScale(glm::vec3(0.1f));
...
#pragma region ModelId
		if (line.find("Model Id") != std::string::npos)
		{
			...
			modelData->model->name = GetString(line);
			continue;
...
#pragma region Position
		...
		if (line.find("Position") != std::string::npos)
		{
			...
			else
			{
				if (section == "Light")
				{
					light->transform.SetPosition(glm::vec3(glm::vec3(values[0], values[1], values[2])));
				}
				else
				{
...
#pragma region Rotation
		if (line.find("Rotation") != std::string::npos)
		{
			...
			else
			{
				if (section == "Light")
				{
					light->transform.SetRotation(glm::vec3(glm::vec3(values[0], values[1], values[2])));
				}
				else
				{
...
#pragma region Scale
		if (line.find("Scale") != std::string::npos)
		{
			...
			if (section == "Light")
			{
				light->transform.SetScale(glm::vec3(glm::vec3(values[0], values[1], values[2])));
			}
			else
			{
